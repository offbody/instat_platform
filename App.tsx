                      ...
                      visibleQuestions.map((metric: any) => (
                        <div key={metric.id} className="p-6 rounded-xl border border-atlassian-border dark:border-atlassian-darkBorder bg-white dark:bg-atlassian-darkSurface shadow-sm hover:shadow-md transition-all group animate-in fade-in slide-in-from-bottom-2 duration-300 relative">
                          ...
                          <div className="flex flex-col gap-4">
                            ...
                            {uploadedFiles[metric.id] && uploadedFiles[metric.id].length > 0 && metric.type !== 'boolean' && (<div className="mt-3 flex justify-start animate-in fade-in slide-in-from-top-1 duration-500"><div className="inline-flex items-center px-4 py-2.5 rounded-lg bg-teal-50 dark:bg-teal-900/30 border border-teal-100 dark:border-teal-800/50"><span className="material-symbols-rounded text-teal-700 dark:text-teal-300 mr-2 text-[18px]" style={{ fontVariationSettings: "'FILL' 1, 'wght' 500, 'GRAD' 0, 'opsz' 24" }}>show_chart</span><span className="text-[10px] font-bold text-teal-700 dark:text-teal-300 uppercase tracking-wide mr-3">Расчёт AI Инстат</span><div className="flex items-baseline gap-1.5 mr-4"><span className="text-sm font-bold text-atlassian-text dark:text-white">{getAiEstimate(metric)}</span>{metric.unit && (<span className="text-[10px] font-medium text-atlassian-text/60 dark:text-white/60">{metric.unit}</span>)}</div><button className="px-3 py-1 bg-teal-600 hover:bg-teal-700 text-white text-[9px] font-black uppercase rounded shadow-sm transition-colors tracking-widest active:scale-95">Применить</button></div></div>)}
        ...
        return (
          <div className="space-y-6 animate-in fade-in duration-500">
             <div className="atl-card p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                   <div className="w-12 h-12 bg-atlassian-brand/10 rounded-xl flex items-center justify-center text-atlassian-brand">
